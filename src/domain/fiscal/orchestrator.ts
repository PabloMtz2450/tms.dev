...
import { assertFiscalProductionReady } from './production-readiness';
...
export async function stampCartaPorteIdempotent(input: {
  ...
  if (acquired.alreadyStamped) return { status: 'ALREADY_STAMPED' as const, fiscalDocument: acquired.document };
  if (!acquired.acquired) return { status: 'IN_PROGRESS' as const, fiscalDocument: acquired.document };
  ...
  try {
    ...
    const definitive = isDefinitiveFailure(error);
    ...
    if (definitive) await markFiscalFailed({ id: acquired.document.id, organizationId: input.organizationId, errorCode: code, safeMessage });
    else await markFiscalRecoveryRequired({ id: acquired.document.id, organizationId: input.organizationId, errorCode: code, safeMessage });

    await appendAuditEvent({
      organizationId: input.organizationId,
      actorId: input.actorId,
      action: definitive ? 'FISCAL_STAMP_FAILED' : 'FISCAL_STAMP_RECOVERY_REQUIRED',
      resourceType: 'FiscalDocument',
      resourceId: acquired.document.id,
      metadata: { code },
    });
