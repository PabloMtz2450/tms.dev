...
export function AppShell({ children, title, subtitle }: { children: ReactNode; title: string; subtitle: string }) {
  return <div className="x-shell">
    <aside className="x-sidebar">
      <Link className="x-brand" href="/" aria-label="TMS XOLUM inicio">
        <span className="x-brand-logo"><Image src="/xolum-symbol-official.png" alt="Símbolo XOLUM" width={54} height={54} priority /></span>
        <span><strong>TMS <em>XOLUM</em></strong><small>Soluciones que realmente ayudan.</small></span>
      </Link>
      <nav className="x-nav" aria-label="Navegación principal">{navGroups.map(group => <section key={group.label}><span className="x-nav-label">{group.label}</span>{group.items.map(([href, icon, label]) => <Link key={`${href}-${label}`} href={href}><span aria-hidden="true">{icon}</span>{label}{label==='Excepciones'&&<b>7</b>}{label==='SafeRoute'&&<b>1</b>}</Link>)}</section>)}</nav>
      <div className="x-value"><span>Valor generado hoy</span><strong>$28,450 <small>MXN</small></strong><em>+12.4% vs ayer</em></div>
      <div className="x-user"><span className="avatar">JP</span><span><strong>Juan Pablo Martínez</strong><small>Torre de Control</small></span><span aria-hidden="true">›</span></div>
    </aside>
